...
MainWindow::onDoTomoPushed() {
    ...
    for(int i=angles.size()-1; i>-1; i--) {
        if(fabsf(angles[i]-180.0f) < fDist) {
            fDist = fabsf(angles[i]-180.0f);
            i180  = i;
        }
    }
    if(fDist > 1.0f) {
        QString sString;
        sString =tr("Distance between specular images = %1°\nIt should be 180°").arg(double(angles[i180]-angles[0]));
        QMessageBox msgBox;
        msgBox.setIcon(QMessageBox::Information);
...
MainWindow::FindCenter(CProjection* pProjection0, CProjection* pProjection180) {
    ...
    memset(m, 0, sizeof(*m)*size_t(imgHeight));
    float *p1, *p2;
    float fMin, fMax, rMin, rMax;
    float rc;

    for(int row=0; row<imgHeight; row++) {
        _x[row] = row-0.5f*imgHeight;
    ...
    for(int i=0; i<imgHeight; i++) {
        if(fabsf(rotationCenter+(tiltAngle)*_x[i]-m[i]) < 2) {
            xx[n] = _x[i];
            mm[n] = m[i];
            n++;
        }
    ...
    QString sString;
    sString = tr("Rotation center %1").arg(double(-rotationCenter));
    qDebug() << sString;
    sString = tr("Tilt Angle %1").arg(-0.5*double(tiltAngle));
    qDebug() << sString;
...
MainWindow::lineRegress(float *x, float *y, int n, float *a, float *b, float *r) {
    ...
    *a = *b = *r = 0;
    ...
    if(fabsf(sxx) == 0.0f) return false;
...
MainWindow::crossCorr(float *y1, float *y2, float *r, int nSamples, int nChan) {
    ...
    memset(r, 0, sizeof(*r)*size_t(nChan));
    for(j=0; j<nSamples; j++) {// t
        mean1 += *(y1+j);
        mean2 += *(y2+j);
    } // for(j=0; j<nSamples; j++)
    mean1 /= float(nSamples);
    mean2 /= float(nSamples);
    ...
    for(i=-nChan/2; i<nChan/2; i++) {// tau
        ...
        *(r+i+nChan/2) /= float(n);// Normalize to N_Points
